...
const _establishSession = async (
    ...
    const url = `https://api.${options.domain}/${CA_API_VERSION}/permission-access/session`;
...
const _getGuestAuthorizeUrl = (session: Session, callbackUrl: string, options: DMESDKConfiguration) => {
    ...
    // tslint:disable-next-line:max-line-length
    return `https://api.${options.domain}/apps/quark/direct-onboarding?sessionExchangeToken=${session.sessionExchangeToken}&callbackUrl=${encodeURIComponent(callbackUrl)}`;
};
...
const _getFileList = async (sessionKey: string, options: DMESDKConfiguration): Promise<string[]> => {
    const url = `https://api.${options.domain}/${CA_API_VERSION}/permission-access/query/${sessionKey}`;
...
const _getFile = async (
    ...
    const url = `https://api.${options.domain}/${CA_API_VERSION}/permission-access/query/${sessionKey}/${fileName}`;
...
const _getSessionAccounts = async (
    ...
    try {
        ...
        const response = await net.get(
            `https://api.${options.domain}/${CA_API_VERSION}/permission-access/query/${sessionKey}/accounts.json`,
            { json: true },
        );
...
const init = (sdkOptions?: Partial<DMESDKConfiguration>) => {
    ...
    if (!isConfigurationValid(options)) {
        throw new ParameterValidationError(
            "SDK options should be object that contains domain property as string",
        );
    }
