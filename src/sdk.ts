...
import { isConfigurationValid, isPlainObject, isSessionValid, isValidString } from "./utils";
...
const _getGuestAuthorizeUrl = (session: Session, callbackUrl: string, options: DMESDKConfiguration) => {
    ...
    // tslint:disable-next-line:max-line-length
    return `${options.baseUrl}/apps/quark/direct-onboarding?sessionExchangeToken=${session.sessionExchangeToken}&callbackUrl=${encodeURIComponent(callbackUrl)}`;
};
...
const init = (sdkOptions?: Partial<DMESDKConfiguration>) => {
    ...
    if (!isConfigurationValid(options)) {
        throw new ParameterValidationError(
            "SDK options should be object that contains baseUrl properties as string",
        );
    }
