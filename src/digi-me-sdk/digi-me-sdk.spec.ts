...
describe("DigiMeSDK", () => {
    describe("Instanced", () => {
        ...
        describe(".withUserAuthorization()", () => {
            test("Returns a `DigiMeSdkAuthorized` instance ", async () => {
                const sdk = new DigiMeSdk(mockSdkOptions);
                const userAuthorization = await UserAuthorization.fromJwt(
                    mockSdkConsumerCredentials.userAuthorizationJwt,
                );

                const result = sdk.withUserAuthorization(userAuthorization, () => {});

                expect(result).toBeInstanceOf(DigiMeSdkAuthorized);
            });
        });
    });
});
