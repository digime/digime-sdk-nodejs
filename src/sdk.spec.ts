...
describe.each<[string, ReturnType<typeof SDK.init>, string]>([
    ...
    (_title, sdk, baseUrl) => {
        ...
        describe("establishSession", () => {
            ...
            it(`Targets API host and version: ${baseUrl}/`, async () => {
        ...
        describe("getGuestAuthorizationUrl", () => {
            describe("Returns a Url where", () => {
                it.each<[string, string, (url: URL) => unknown]>([
                    ["Protocol", new URL(baseUrl).protocol, (url) => url.protocol],
                    ["Host", new URL(baseUrl).host, (url) => url.host],
                    ["Pathname", `${new URL(baseUrl).pathname}/apps/quark/direct-onboarding`, (url) => url.pathname],
                    [
        ...
        describe("getSessionAccounts", () => {
            it(`Requests target API host and version: ${baseUrl}`, async () => {
                ...
                nock(`${baseUrl}`)
                    .get(`/permission-access/query/test-session-key/accounts.json`)
                    .reply(200, {
                        fileContent: {
                            accounts: [{
                                name: "test-account",
