...
describe("init", () => {
    ...
    describe("Throws ParameterValidationError when options (first parameter) is", () => {
        ...
        it.each([true, false, null, [], 0, NaN, "", (): null => null, Symbol("test"), { domain: null }])(
            "%p",
            (options: any) => {
...
describe.each<[string, ReturnType<typeof SDK.init>, string, string]>([
    ["Default exported SDK", SDK, "api.digi.me", "v1.0"],
    ["Custom SDK", customSDK, "api.digi.test", "v1.0"],
])(
    "%s",
    (_title, sdk, host, version) => {
        ...
        describe("establishSession", () => {
            ...
            it(`Targets API host and version: ${host}/`, async () => {
                const callback = jest.fn();
                const scope = nock(`https://${host}`).post(new RegExp(`^/(.*)`)).reply(200);
        ...
        describe("getSessionAccounts", () => {
            it(`Requests target API host: https://${host}`, async () => {

                nock(`https://${host}`)
                    .get(`/${version}/permission-access/query/test-session-key/accounts.json`)
                    .reply(200, {
                        fileContent: {
                            accounts: [{
                                name: "test-account",
